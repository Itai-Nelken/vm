...
void arrayPop(Array *a, struct operation *op) {
    op->instruction=a->data[--a->current].instruction;
    op->arg1=a->data[a->current].arg1;
    op->arg2=a->data[a->current].arg2;
}

int arrayGet(Array *a, unsigned int index, struct operation *op) {
    if(index > a->size) {
...
void dumpArray(Array *a) {
    for(unsigned int index=0; index<(a->current); index++) {
